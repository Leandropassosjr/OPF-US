...
from undersampling.opf_us_balance import OpfUSBalance
...
def perform_under(**kwargs):
    ...
    opf_us_obj.saveDataset(X_res, y_res, pathDataset, ds)
...
for dsds in range(len(datasets)):
    ...
    for ff in range(len(folds)): 
       ...
       #2st case: remove samples from majoritary class with negative or zero scores
        ...
        perform_under(us_obj=opf_us_balanced, output=output, X=X, y=Y, X_test=X_test, y_test=Y_test,
                      fold=f, ds=ds, majority_class=majority_class, minority_class=minority_class, 
                      exec_time=end_time)        
